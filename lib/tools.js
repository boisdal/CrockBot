...
import { oppositeOperators } from './constants';
...
const requirementToHuman = (req, negation=false) => {
    ...
    if (req.test == NAMETest) {
        if (negation) {
            return `No ${req.name} or its cooked variant.`;
        } else {
            return `At least 1 ${req.name} or its cooked variant.`;
        }
    }
    if (req.test == NOTTest) {

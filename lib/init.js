...
export const initConfig = (bot) => {
    const configList = fs
		.readdirSync('./config')
		.filter((file) => file.endsWith('.json'));
    for (let config of configList) {
        let name = config.slice(0, -5);
        bot.config = {}
        bot.config[name] = {};
        fs.readFile(`./config/${config}`, 'utf8', function readFileCallback(err, data){
            if (err){
                console.log(err);
            } else {
            let obj = JSON.parse(data);
            bot.config[name].data = obj;
        }});
        bot.config[name].write = function() {
            fs.writeFile(`./config/${config}`, JSON.stringify(this.data), 'utf8', function(err) {
                if(err) {

...
export const createEmbedForCook = (bot, recipeArray, ingredientlist, ephemeral) => {
    let embedArray = [];
    let fileArray = [];
    ...
    for (let i=2; i<recipeArray.length; i++) {
        ...
        if (recipe.priority == bestPriority) {
            ...
            let ingredientsString = ingredientlist.map((i) => bot.food.byId(i).name).join(', ');
...
export const createEmbedForRecipe = (bot, recipe, ephemeral) => {
    let reqString = requirementArrayToHuman(bot, recipe.requirements);
    let resultAttachment = getAttachmentFromName(recipe.name);
    let perishDays = recipe.perish / total_day_time;
    let exampleString = '';
    if (recipe.example) {
        exampleString = `${bot.food.byId(recipe.example[0]).name}, ${bot.food.byId(recipe.example[1]).name}, ${bot.food.byId(recipe.example[2]).name} and ${bot.food.byId(recipe.example[3]).name}.`
    } else {
        exampleString = `Currently, no recipe has been found to craft this recipe.`
    }
    let embed = new EmbedBuilder()
        .setColor(getColorFromHHS(recipe.health, recipe.hunger, recipe.sanity))
        ...
        .addFields(
            ...
            { name: 'Ingredient example list', value: exampleString},

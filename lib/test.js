...
import { isBestStat, isStat, pl } from './utils.js';
...
export const getRecipesFromIngredients = (ingredient_array) => {
    return getRecipes([food[ingredient_array[0]], food[ingredient_array[1]], food[ingredient_array[2]], food[ingredient_array[3]]])
}
...
export const getFoodItemArray = () => {
    ...
    for (const key in foodChoices) {
        foodArray.push({ name: food[key].name, value: food[key].id })
    }
    return foodArray
}
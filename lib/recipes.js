...
import { food, uniqueFood } from '#lib/food';
import {
	AND,
	COMPARE,
	NAME,
...
import { pl, stats } from '#lib/utils';
...
for (const key in recipes) {
	...
	if (recipes[key].requirements) {
		...
		if (recipes[key].mode) {
			recipes[key].modeNode = '[tag:' + recipes[key].mode + '|img/' + modes[recipes[key].mode].img + ']';
		}

		recipes[key].requires =requirements.join('; ');
	}
